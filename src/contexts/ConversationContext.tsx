...
export const ConversationProvider: React.FC<ConversationProviderProps> = ({ children, onSearch }) => {
  // Persist messages in localStorage
  const [storedMessages, setStoredMessages] = useLocalStorage<ChatMessage[]>('nexus-conversation', []);
  ...
  const [categoryContext, setCategoryContext] = useState<string | null>(null);
  ...
  const handleSubmit = useCallback(async (e?: React.FormEvent) => {
    ...
    try {
      // If we have a category context, include it in the system prompt
      let systemContext = '';
      if (categoryContext) {
        systemContext = `The user is currently exploring the "${categoryContext}" topic. Keep your responses focused on this subject area when appropriate.`;
      }
  ...
  }, [conversationHistory, currentMessage, generateRelatedQuestions, onSearch, categoryContext]);
  ...
  const handleRegenerateMessage = async (messageId: string) => {
    ...
    try {
      ...
      const diversityPrompt = `${systemContext} Please provide a different perspective or approach than previous responses. Use different examples, phrasing, and structure. If this is a regeneration request, avoid repeating the same content or examples from previous responses. Temperature has been increased to encourage creativity.`;
